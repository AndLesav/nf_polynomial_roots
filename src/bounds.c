...
/* evaluation size root */
Precision_eval_eq(h, pol_eq, field_dim)=  {
  return(round(field_dim^2/35)+field_dim*round(log(norml2(h))/(2*log(2)*poldegree(pol_eq)*sqrt(field_dim))));
};
...
Rel_precision_eval(pol_vec, equation, {version=1}) = {
  my(dg, da, de, dp, pol_e, ind, base, M, Minv, Binv, pol_mink, Bpol, n_gp, A, prec, field_dim, r1, n, m, C, B_vec);
