...
/* HNF_Is_vector_new := function(H, v) */
/* end function; */
...
Lattice_intersection := function(M1, M2)
    /* M := HermiteForm(M1); */
    /* N := HermiteForm(M2); */
    /* L := BasisMatrix(Lattice(M1) meet Lattice(M2)); */
    /* return L; */
...
Orthogonality_parameters := function(LU, precision_float, rank, vol)
    R := rank;
    LU := ChangeRing(LU, RealField(precision_float));
    ma, mi, norms := My_row_maxnorm(LU);
    d_0 := RealField(4)!(mi/Root(vol, rank));
    d_n := RealField(4)! Root(&*norms/vol, rank);
    return d_0, d_n;
end function;
...
get_nbprimes := function(K: deg_eq:=1)
    if deg_eq ne 1 then
	B := AbsoluteDegree(K) * deg_eq;
    else
	B := AbsoluteDegree(K) * 20;
    end if;
    if B lt 128 then
	nb := 5;
    elif B lt 1024 then
    ...
    return Max(nb, 2*AbsoluteDegree(K));
end function;

max_inertial_degree := function(K, p)
    decomp := DecompositionType(K, p);
    L := [decomp[i,1] : i in [1..#decomp]];
    return Max(L);
end function;

is_good_prime := function(K, p, maxf)
    id := max_inertial_degree(K, p);
    if AbsoluteDegree(K) eq id then
	return true, id;
    else
	return id lt maxf, id;
    end if;
end function;


/* rough simulation of Pari/Gp process  */
find_best_inertial_degree := function(K: deg_eq:=1)
    disc := Abs(Discriminant(DefiningPolynomial(K)));
    disc := Integers()!(Denominator(disc) * disc);
	...
	if (disc mod p) ne 0 then
	    init, best_f := is_good_prime(K, p, maxf);
	end if;
    end while;

    /* then start the process */
    count := get_nbprimes(K: deg_eq:=deg_eq)-1;
    while ((count gt 0) and (best_f ne dim)) do
	p := NextPrime(p);
	if (disc mod p) ne 0 then
	    count -:= 1;
	    bool, id := is_good_prime(K, p, maxf);
	    if bool then
		best_f := Max(id, best_f);
	    end if;
	end if;
    end while;
    return best_f;
...
/* rough estimate of whether K is good field or not */
Is_good_field := function(K: deg_eq:=1)
    dim := AbsoluteDegree(K);
    best_f := find_best_inertial_degree(K: deg_eq:=deg_eq);
    print(best_f);
    bool := (best_f eq dim);
    return bool, best_f;
end function;
