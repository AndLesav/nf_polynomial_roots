...
# data file
str_file = '../data/cf_tw_sti_saturation_bad_' + type_prime + '_' + str(size_primes) + '_cc_embeddings'
...
for m in range(10, 400):
    ...
    for i in range(H.nrows()):
        ...
        t = cputime()
        res = cf_sqrtn_fam([C], er, K);
        print("found a sol?: ", len(res))
        res = res[0]; t = cputime(t);
        t_cc += t;
        print("\tTime Root: {:.4f}s".format(t));
        s_cc.append(res);
        
        # Check
        ls_cc = logarg_set(s_cc[-1], phi);
        print("\tPari/Gp nfroots: Check log, arg mod 2pi/e: \x1b[{}OK\x1b[0m"
              .format("32m" if logarg_is_equal(ls_cc,s[i],argmod=er) else "31mN"));
        # The assert are here to quantify how far are the values for diagnostic
        assert(fp.fp_assert_equal("", ls_cc.inf, s[i].inf, target=1000));
        assert(fp.fp_assert_zero("", [min(_th.abs(), 1-_th.abs()) for _th in logarg_reduce_args(er*ls_cc.args) - logarg_reduce_args(er*s[i].args)], target=1000));

    t_cc = "%.2f" % t_cc
    hs = "%.2f" % log(B,2)
    f = open(str_file, 'a')
    f.write(str(H.nrows()) + "\t" + str(er) + "\t" + hs + "\t" +  t_cc + "\n")
    f.close()
