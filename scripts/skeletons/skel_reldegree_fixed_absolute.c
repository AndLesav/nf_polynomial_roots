...
read(strprintf("../inputs/input_abs_reldeg_fixed_%d_%d.c", DIM_EXT, DIM_G_M));
...
default(nbthreads, 1);
{
  ExperimentsRelDegreeFixed_LLL_abs(DIM_EXT, DIM_G_m, DIM_G_M, DEGREE_EQ,\
				    vector_field_abs, NUMBER_TESTS, SIZE_ROOTS,\
				    VERSION);
};
